-- Distinguish home-clan vs opponent attacks for correct per-player war defense stats.

ALTER TABLE public.war_attacks
    ADD COLUMN IF NOT EXISTS is_home_attacker BOOLEAN;

COMMENT ON COLUMN public.war_attacks.is_home_attacker IS
    'True when attacker is on the tracked clan side (CoC war payload clan); false when attacker is opponent. NULL = legacy row before backfill via ingestion.';
...
COMMENT ON FUNCTION public.war_player_leaderboard_stats(text) IS
    'Per-player war stats for ended wars of one clan; requires war_attacks.is_home_attacker set.';
...
COMMENT ON FUNCTION public.war_player_attack_history(text, text) IS
    'Offense and defense war attack rows for one player in one clan (ended wars only).';

GRANT EXECUTE ON FUNCTION public.war_player_leaderboard_stats(text) TO service_role;
GRANT EXECUTE ON FUNCTION public.war_player_attack_history(text, text) TO service_role;
