-- Missed slots must subtract every home offensive swing (including farming dips).
-- Previously missed used only non-farming rows, so farming attacks still appeared as
-- offense while synthetic "missed" rows duplicated the same wars.
...
AS $$
    ...
    offense_swings_agg AS (
        SELECT
            ow.tag,
            count(*)::bigint AS swing_count
        FROM off_window ow
        GROUP BY ow.tag
    ),
    defense_agg AS (
    ...
    SELECT
        ...
        greatest(
            0,
            COALESCE(ps.expected_attacks, 0) - COALESCE(os.swing_count, 0)
        )::bigint AS attacks_missed
    FROM all_tags t
    LEFT JOIN offense_agg o ON o.tag = t.tag
    LEFT JOIN offense_swings_agg os ON os.tag = t.tag
...
COMMENT ON FUNCTION public.war_player_leaderboard_stats(text, integer) IS
    'Per-player war stats for one clan (ended wars). p_max_attacks NULL = all attacks; else last N offensive swings and last N defensive rows (by recency). Omits farming from offense/defense averages and counts columns; missed = expected slots minus all home offensive swings in window (farming counts as used).';
...
AS $$
    ...
    war_miss AS (
        SELECT
            sw.id AS war_id,
            greatest(0, sw.apm - COALESCE(us.used_n, 0))::integer AS missed_n
        FROM scoped_wars sw
        INNER JOIN player_scoped_war_ids psw ON psw.war_id = sw.id
        LEFT JOIN used_swings us ON us.war_id = sw.id
    ),
    missed_expanded AS (
        SELECT
            'missed'::text AS kind,
...
COMMENT ON FUNCTION public.war_player_attack_history(text, text, integer) IS
    'War rows for one player: offense (incl. farming), synthetic missed slots, defense. Missed = apm minus count of home offensive rows (farming counts). p_max_attacks limits last N offense and last N defense rows; missed only for wars in that scope.';
