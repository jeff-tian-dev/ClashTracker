-- Replace last-N-wars window with last-N-attacks (per player: offensive swings + defensive
-- occurrences, ranked by war start_time / attack_order). History gains synthetic "missed"
-- rows (kind = missed); they do not affect offense/defense aggregates.

DROP FUNCTION IF EXISTS public.war_player_leaderboard_stats(text, integer);
DROP FUNCTION IF EXISTS public.war_player_attack_history(text, text, integer);
...
AS $$
    ...
    off_window AS (
        ...
        FROM off_ranked o
        WHERE p_max_attacks IS NULL OR o.rn_off <= p_max_attacks
    ),
    def_ranked AS (
        SELECT
    ...
    def_window AS (
        ...
        FROM def_ranked d
        WHERE p_max_attacks IS NULL OR d.rn_def <= p_max_attacks
    ),
    offense_agg AS (
        SELECT
            ow.tag,
...
AS $$
    ...
    off_scoped AS (
        SELECT *
        FROM off_src
        WHERE p_max_attacks IS NULL OR rn_off <= p_max_attacks
    ),
    def_src AS (
        SELECT
            wa.war_id,
    ...
    def_scoped AS (
        SELECT *
        FROM def_src
        WHERE p_max_attacks IS NULL OR rn_def <= p_max_attacks
    ),
